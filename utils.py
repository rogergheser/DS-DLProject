...
def caption_report(images, caption_logits, ice_scores, label, outputs, caption_prediction, id2class, idx):
    """
    ...
    for i, image in enumerate(images[:9]):
        ...
        plt.grid()
        plt.barh(y, cap_probabilities[i], color="red")
        plt.barh(y, ice_probabilities[i], color="green")
        plt.gca().invert_yaxis()
        plt.gca().set_axisbelow(True)
